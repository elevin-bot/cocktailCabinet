...
router.post("/api/cabinet", async (req, res) => {
  ...
  try {
    const sql =
      "INSERT INTO cabinet_contents(user_id, liquor_id, volume) VALUES($1, $2, $3)";
...
router.put("/api/cabinet", async (req, res) => {
  ...
  try {
    const sql =
      "UPDATE cabinet_contents set volume = $3) WHERE user_id = $1 and liquor_id = $2";
...
router.delete("/api/cabinet/:id", (req, res) => {
  ...
  db.query(
    "DELETE from cabinet_contents where user_id = $1 and liquor_id = $2",
...
router.post("/api/cocktail", async (req, res) => {
  ...
  try {
    const sql =
      "INSERT INTO cocktail(name, description, procedure) VALUES($1, $2, $3) RETURNING id";
    ...
    cocktailIngredients.forEach((element) => {
      const sql =
        "INSERT INTO cocktail_ingredients(cocktail_id, liquor_id, volume) VALUES($1, $2, $3)";
      const dbRes = db.query(sql, [
        cocktail_id,
        element.liquor_id,
        element.volume,
      ]);
